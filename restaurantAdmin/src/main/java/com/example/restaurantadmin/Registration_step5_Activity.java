...
//	}
	...
	private void setlistner() {
		...
		ch_terms.setOnClickListener(new OnClickListener() {
			...
			try {
				...
				try {
					...
					String str_response = con.connection(
							Registration_step5_Activity.this,
...
//					Log.i(getString(R.string.str_please_wait), e + "");
		...
		protected void onPostExecute(Void result) {
			...
			try {
				...
				System.out.println("succysssai"+jsonSuccessStr);
				...
				if (jsonSuccessStr.equalsIgnoreCase("true")) {
					...
					if (Error != null) {
						...
						if (Error.has("contact_email")) {
							Toast.makeText(
									getApplicationContext(),
									Error.getJSONArray("contact_email").get(0)
											.toString(), Toast.LENGTH_LONG)
									.show();
