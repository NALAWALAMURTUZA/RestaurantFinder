...
public class Registration_step4_Activity extends Activity {
	...
	public static TextView txv_package1_text1, rf_regi_step4_cancel;
...
//		LanguageConvertLocalPrefernce.loadLocale(getApplicationContext());
			...
			for (int i = 0; i < Global_variable.array_Package.length(); i++) {
				...
				try {
					...
					if (str_price.equalsIgnoreCase("0.00")) {
						...
						txv_package2_text2.setText("lei" + " " + str_price);
					}
					if (str_online_order_limit.equalsIgnoreCase("0")) {
						txv_package2_text3
								.setText(getString(R.string.str_we_can_accept_unlimited));
	...
	private void initialization() {
		...
		rf_regi_step4_cancel = (TextView) findViewById(R.id.rf_regi_step4_cancel);
		rf_regi_step4_cancel.setText(R.string.str_rf_regi_step4_cancel);
